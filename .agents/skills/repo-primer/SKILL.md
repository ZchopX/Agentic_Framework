...
## Workflow
1. Map repository structure with fast file discovery. Preferred command: `rg --files --hidden -g '!.git' -g '!.agent/plans/archive/**'`. Fallback (if `rg` is unavailable): run an equivalent recursive file walk that excludes `.agent/plans/archive/`.
2. Read core context docs first: `AGENTS.md`, `.agent/templates/AGENTS-template.md`, root `README*`, `.agent/PRD.md`.
...
- Do not treat `.agent/plans/archive/**` as active planning scope; include it only when the user explicitly asks for archived plans.
