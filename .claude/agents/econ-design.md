...
- **Threats to Validity**: known risks and which the strategy does or does
  not address.
- **Required Diagnostics**: the specific tests `econ-test-writer` must
  implement (e.g., Hausman, Breusch-Pagan, unit-root tests, serial-correlation
...
If you are invoked again because `econ-triage` classified a downstream
